...
def process_raw_tx_data(tx_list):
    """
    Processes a list of raw transaction hex strings entirely in memory.
    """
    results = []
    print(f"[*] Processing {len(tx_list)} embedded transactions...")
    for idx, line in enumerate(tx_list, 1):
        line = line.strip()
        if not line:
            continue
        ...
        if len(line) > 100 and all(c in '0123456789abcdefABCDEF' for c in line):
            ...
            if extracted:
                ...
                print(f"[-] Item {idx}: Could not parse or compute Z from raw hex.")
        else:
...
if __name__ == "__main__":
    # Insert your raw transaction hex strings directly into this list
    RAW_TRANSACTIONS = [
        "0100000001...", # Example placeholder 1
        "0100000002...", # Example placeholder 2
    ]
    
    extracted_data = process_raw_tx_data(RAW_TRANSACTIONS)
    for data in extracted_data:
        print(f"Extracted -> R: {hex(data[0])}, S: {hex(data[1])}, Z: {hex(data[2])}")
